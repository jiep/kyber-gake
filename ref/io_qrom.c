...
int read_keys(char* filename, keys_qrom_t* data) {
  ...
  if (fread(data, sizeof(keys_qrom_t), 1, fp)) {
    return 1;
  }
  fclose(fp);
  return 0;
...
int read_ca_data(char* filename, int* parties, ca_qrom_public* data) {
  ...
  if (fread(parties, sizeof(int), 1, fp)) {};
  if (fread(data, sizeof(ca_qrom_public), *parties, fp) != ((size_t) *parties)) {
    return 1;
  }
  fclose(fp);
  return 0;
...
int get_pk(char* ip_str, uint8_t* pk, ca_qrom_public* data, int length) {
  ...
  for (int i = 0; i < length; i++) {
    if(memcmp(data[i].ip, ip, sizeof(ip_t)) == 0) {
      memcpy(pk, data[i].public_key, KYBER_INDCPA_PUBLICKEYBYTES);
      return 0;
    }
  }
  return 1;
...
int write_keys(keys_qrom_t* key, char* outfile) {
  ...
  fwrite(key, sizeof(keys_qrom_t), 1, fp);
...
int write_ca_info(ca_qrom_public* pps, int parties, char* outfile) {
  FILE* fp;
  fp = fopen(outfile, "wb");
  if (fp == NULL)
    return 1;
  ...
  fwrite(pps, sizeof(ca_qrom_public), parties, fp);
