...
int main() {
  ...
  unsigned char tag[AES_256_CCM_TAG_LENGTH];

  int ciphertext_len = ccm_encrypt(plaintext, plaintext_len,
                                   aad, aad_len,
                                   key,
                                   iv,
                                   ciphertext,
  ...
  printf("Tag: ");
  print_key(tag, AES_256_CCM_TAG_LENGTH);

  unsigned char plaintext_dec[1000];

  plaintext_len = ccm_decrypt(ciphertext, ciphertext_len,
                              aad, aad_len,
                              tag,
                              key,
                              iv,
